...
CACHE_PLIES = 16  # only shallow positions recur across games often enough to be worth caching
...
class Game:
    ...
    def expand_and_backup(self, logits, value):
        """Expand from network output; returns (priors, value) for the eval cache."""
        legal = self.pending[-1].legal
        picked = logits[ROW[legal], COL[legal]].astype(np.float64)
        p = np.exp(picked - picked.max())
        p /= p.sum()
        self.expand_with(p, float(value))
        return p, float(value)

    def expand_with(self, p, value):
        path = self.pending
        leaf = path[-1]
        legal = leaf.legal
...
def worker(wid, net_path, out_dir, n_games, sims, batch_games, seed, progress, opts):
    ...
    ct = dict(evals=0, batches=0, moves=0, positions=0, games=0, hits=0, lookups=0)
    cache, cache_max = {}, opts["cache"]
    hit_ply = np.zeros((2, 90), dtype=np.int64)  # [lookups, hits] by ply
    ...
    while games:
        ...
        if bench and not measuring and now >= warm_until:
            tm = dict.fromkeys(keys, 0.0)
            ct = dict.fromkeys(ct, 0)  # hit_ply keeps counting from the start: the opening is where hits live
            measuring, t_measure = True, now
        if bench and measuring and now - t_measure >= bench:
            ct["hit_ply"] = hit_ply
            progress.put(("stats", wid, now - t_measure, tm, ct))
            return
        leaves, owners, keys_b = [], [], []
        t0 = clock()
        enc = 0.0
        for g in games:
            for _ in range(8 if cache_max else 4):  # terminal leaves and cache hits back up for free
                if cache_max and g.search_done():
                    break
                leaf = g.select()
                if leaf is None:
                    continue
                ply = g.ply + len(g.pending) - 1
                if cache_max and ply < CACHE_PLIES:
                    key = bytes(leaf.u.state)
                    hit = cache.get(key)
                    ct["lookups"] += 1
                    hit_ply[0, ply] += 1
                    if hit is not None:
        ...
        if leaves:
            ...
            ct["evals"] += len(leaves)
            ct["batches"] += 1
            for i, (g, lg, v) in enumerate(zip(owners, logits, values)):
                out = g.expand_and_backup(lg, v)
                if cache_max and keys_b[i] is not None and len(cache) < cache_max:
                    cache[keys_b[i]] = out  # full cache: keep what it has (shallow positions recur, deep ones rarely)
            t1 = clock()
            tm["backup"] += t1 - t2
        nxt = []
        for g in games:
...
def parse_opts(argv):
    opts = dict(backend="torch", fp16=False, cap=1.0, cheap_sims=None, bench=0, cache=0)
    pos, i = [], 0
    while i < len(argv):
        a = argv[i]
        if a == "--backend":
...
def report_bench(stats, workers, opts, sims, batch_games):
    ...
    if tot["lookups"]:
        hp = tot["hit_ply"]
        print(f"  eval cache: {tot['hits'] / tot['lookups']:.1%} hit rate over {tot['lookups']:,} lookups; by leaf ply: "
              + ", ".join(f"{lo}-{lo + 9} {hp[1, lo:lo + 10].sum() / max(1, hp[0, lo:lo + 10].sum()):.0%}"
                          f" ({hp[0, lo:lo + 10].sum() / hp[0].sum():.0%} of lookups)"
                          for lo in range(0, 90, 10) if hp[0, lo:lo + 10].sum()), flush=True)
